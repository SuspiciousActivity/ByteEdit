...
public class WrappedSyntaxView extends BoxView implements TabExpander, RSTAView {
	...
	protected int calculateBreakPosition(int p0, Token tokenList, float x0) {
		...
		while (t != null && t.isPaintable()) {
			// FIXME: Replace the code below with the commented-out line below.
			// This will
			// allow long tokens to be broken at embedded spaces (such as
			// MLC's). But it
			...
			x0 += tokenWidth;
			p += t.length();
			// System.err.println("*** *** *** token fit entirely (width==" +
			// tokenWidth + "), adding " + t.textCount + " to p, now p==" + p);
			t = t.getNextToken();
		}
	 ...
	 *
	 * @param painter
	 *            The painter to use to render tokens.
	 * @param g
	...
	protected void drawView(TokenPainter painter, Graphics2D g, Rectangle r, View view, int fontHeight, int y, int line) {
		...
		// System.err.println("drawSeg=='" + drawSeg + "' (p0/p1==" + p0 + "/" +
		// p1 + ")");
	 ...
	 */
	protected void drawViewWithSelection(TokenPainter painter, Graphics2D g, Rectangle r, View view, int fontHeight, int y, int selStart,
			int selEnd) {
		float x = r.x;
			...
			while (token != null && token.isPaintable() && token.getEndOffset() - 1 < p) {// <=p)
				...
				if (token.containsPosition(selStart)) {
					...
					if (selCount == token.length()) {
						...
						x = painter.paintSelected(tempToken, g, x, y, host, this, useSTC);
						tempToken.textCount = token.length();
	...
	public void paint(Graphics g, Shape a) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (tempRect.intersects(clip)) {
				...
				int endOffset = lineElement.getEndOffset() - 1; // Why always
																// "-1"?
				View view = getView(i);
	...
	private void setSegment(int p0, int p1, Document document, Segment seg) {
		try {
			// System.err.println("... in setSharedSegment, p0/p1==" + p0 + "/"
			// + p1);
			document.getText(p0, p1 - p0, seg);
			// System.err.println("... in setSharedSegment: s=='" + s + "';
			// line/numLines==" + line + "/" + numLines);
	...
	void updateChildren(DocumentEvent e, Shape a) {
		...
		if (e.getType() == DocumentEvent.EventType.CHANGE) {
			// System.err.println("Updating the damage due to a CHANGE
			// event...");
	...
	public int viewToModel(float x, float y, Shape a, Position.Bias[] bias) {
		...
		if (v != null) {
			...
			// Code folding may have hidden the last line. If so, return the
			// last
	...
	class WrappedLine extends View {
		...
		final int calculateLineCount() {
			...
			// System.err.println(">>> calculateLineCount: " + startOffset + "-"
			// + p1);
			 ...
			 * }
			 * System.err.
			 * println(">>> >>> calculated number of lines for this view (line "
		...
		public int viewToModel(float fx, float fy, Shape a, Position.Bias[] bias) {
			...
			if (y < alloc.y) {
				...
				while (p0 < p1) {
					...
					// line.
					TokenSubList subList = TokenUtils.getSubTokenList(tlist, p0, WrappedSyntaxView.this, textArea, alloc.x,
							lineCountTempToken);
