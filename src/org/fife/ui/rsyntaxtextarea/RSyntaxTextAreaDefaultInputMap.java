...
public class RSyntaxTextAreaDefaultInputMap extends RTADefaultInputMap {
	...
	public RSyntaxTextAreaDefaultInputMap() {
		...
		if (os == RSyntaxUtilities.OS_WINDOWS || os == RSyntaxUtilities.OS_MAC_OSX) {
			...
			// ToggleCommentAction.
			// It triggers both KEY_PRESSED ctrl+'/' and KEY_TYPED '/' events
			// when the
			// user presses ctrl+'/', but Windows and OS X do not. If we try to
			// "move"
			// the KEY_TYPED event for '/' to KEY_PRESSED, it'll work for Linux
			// boxes
			// with QWERTY keyboard layouts, but non-QUERTY users won't be able
			// to type
			// a '/' character at all then (!). Rather than try to hack together
			// a
