...
public abstract class TokenMakerBase implements TokenMaker {
	...
	/**
	 * "0" implies this is the "main" language being highlighted. Positive values
	 * imply various "secondary" or "embedded" languages, such as CSS or JavaScript
	 * in HTML. While this value is non-zero, tokens will be generated with this
	 * language index so they can (possibly) be painted differently, so "embedded"
	 * languages can be rendered with a special background.
	 */
	...
	public void addToken(char[] array, int start, int end, int tokenType, int startOffset, boolean hyperlink) {
		if (firstToken == null) {
			...
			TokenImpl next = tokenFactory.createToken(array, start, end, startOffset, tokenType);
			if(currentToken == null || next == null ) return;
			currentToken.setNextToken(next);
			previousToken = currentToken;
			currentToken = next;
		}
	 ...
	 * default implementation returns <code>null</code>. Subclasses can override.
	 *
	 ...
	 * enabled for the current programming language. The default implementation
	 * returns true if <tt>type</tt> is {@link Token#IDENTIFIER}. Subclasses can
	 * override this method to support other token types, such as
	 * {@link Token#VARIABLE}.
	 *
	 * @param type The token type.
	...
	/**
	 * The default implementation returns <code>false</code> always. Languages that
	 * wish to better support auto-indentation can override this method.
	 *
	 ...
	 * specific language. The default implementation returns <code>true</code> for
	...
	/**
	 * The default implementation returns <code>false</code> always. Subclasses that
	 * are highlighting a markup language should override this method to return
	 * <code>true</code>.
	 *
	 ...
	 * have to be called by the programmer, as it is automatically called whenever
	 * the user calls {@link #getLastTokenTypeOnLine(Segment, int)} or
	...
	/**
	 * Sets the language index to assign to tokens moving forward. This property is
	 * used to designate tokens as being in "secondary" languages (such as CSS or
	 * JavaScript in HTML).
	 *
