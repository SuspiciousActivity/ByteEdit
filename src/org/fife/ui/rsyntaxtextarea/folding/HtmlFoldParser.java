...
public class HtmlFoldParser implements FoldParser {
				...
				while (t != null && t.isPaintable()) {
					...
					if (!inSublanguage) {
						if (t.getType() == Token.COMMENT_MULTILINE) {
							...
							if (inMLC) {
								// Found the end of the MLC starting on a
								// previous line...
								if (t.endsWith(MLC_END)) {
									int mlcEnd = t.getEndOffset() - 1;
									currentFold.setEndOffset(mlcEnd);
									Fold parentFold = currentFold.getParent();
									// Don't add fold markers for single-line
									// blocks
									if (currentFold.isOnSingleLine()) {
										removeFold(currentFold, folds);
									}
							...
							// Starting a JSP comment that ends on a later
							// line...
							else if (language == LANGUAGE_JSP && t.startsWith(JSP_COMMENT_START) && !t.endsWith(JSP_COMMENT_END)) {
								if (currentFold == null) {
									currentFold = new Fold(FoldType.COMMENT, textArea, t.getOffset());
									folds.add(currentFold);
								} else {
						...
						// If we're starting a new tag...
							...
							if (isFoldableTag(tagNameToken)) {
								...
								if (tci.line == -1) { // EOF reached before end
														// of tag
									return folds;
								}
								// We have found either ">" or "/>" with tci.
								// System.out.println(line + ", "+ tci + ", " +
								// t);
	 ...
	 * @param info
	 *            On return, information about the closing of the tag is
	 ...
	 * @param tagNameToken
	 *            The tag name of the most recently parsed closing
	 *            tag.
	...
	private static class TagCloseInfo {
		...
		public String toString() {
			return "[TagCloseInfo: " + "closeToken=" + closeToken + ", line=" + line + "]";
		}
	}
}
