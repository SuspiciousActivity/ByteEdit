					...
					if (getFoldableMultiLineComments() && t.isComment()) {
						...
						if (java) {
							if (importStartLine > -1) {
								if (lastSeenImportLine > importStartLine) {
									Fold fold = null;
									// Any imports found *should* be a top-level
									// fold,
									// but we're extra lenient here and allow
									// groups
									// of them anywhere to keep our parser
									// better-behaved
									// if they have random "imports" throughout
									// code.
									if (currentFold == null) {
						...
						if (inMLC) {
							...
							if (t.getType() != Token.COMMENT_EOL && !t.endsWith(C_MLC_END)) {
								// System.out.println("Starting MLC at: " +
								// t.offset);
								inMLC = true;
						...
						if (currentFold != null) {
							...
							// System.out.println("... Adding regular fold at "
							// + t.offset + ", parent==" + parentFold);
