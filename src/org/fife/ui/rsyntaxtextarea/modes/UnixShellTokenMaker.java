...
public class UnixShellTokenMaker extends AbstractTokenMaker {
	...
	protected final String shellVariables = "#-?$!*@_"; // Characters that are
														// part of "$<char>"
														// shell variables;
														// e.g., "$_".
		...
		for (int i = offset; i < end; i++) {
			...
			case Token.NULL:
				...
				case '\t':
					...
					break;
				case '`':
					if (backslash) { // Escaped back quote => call '`'
										// an identifier..
					...
					break;
				case '"':
					if (backslash) { // Escaped double quote => call '"'
										// an identifier..
						addToken(text, currentTokenStart, i, Token.IDENTIFIER, newStartOffset + currentTokenStart);
						backslash = false;
					} else {
					...
					break;
				case '\'':
					if (backslash) { // Escaped single quote => call
										// '\'' an identifier.
						addToken(text, currentTokenStart, i, Token.IDENTIFIER, newStartOffset + currentTokenStart);
						backslash = false;
					} else {
				...
				case '\\':
					...
					break;
				case '$':
					if (backslash) { // Escaped dollar sign => call '$'
										// an identifier..
						addToken(text, currentTokenStart, i, Token.IDENTIFIER, newStartOffset + currentTokenStart);
						backslash = false;
					} else {
			...
			case Token.WHITESPACE:
				...
				case '\\':
					...
					currentTokenType = Token.NULL;
					backslash = true; // Previous char whitespace =>
										// this must be first backslash.
					break;
				case '`': // Don't need to worry about backslashes as
			...
			case Token.IDENTIFIER:
				...
				case '\t':
					...
					currentTokenType = Token.WHITESPACE;
					break;
				case '/': // Special-case to colorize commands like
							// "echo" in "/bin/echo"
					addToken(text, currentTokenStart, i, Token.IDENTIFIER, newStartOffset + currentTokenStart);
				...
				case '`': // Don't need to worry about backslashes as
					...
					break;
				case '"': // Don't need to worry about backslashes as
							// previous char is non-backslash.
					addToken(text, currentTokenStart, i - 1, Token.IDENTIFIER, newStartOffset + currentTokenStart);
					currentTokenStart = i;
					currentTokenType = Token.LITERAL_STRING_DOUBLE_QUOTE;
					backslash = false;
			...
			case Token.LITERAL_NUMBER_DECIMAL_INT:
				...
				default:
					...
					// Otherwise, remember this was a number and start
					// over.
					addToken(text, currentTokenStart, i - 1, Token.LITERAL_NUMBER_DECIMAL_INT,
							newStartOffset + currentTokenStart);
					i--;
			...
			case Token.VARIABLE:
				...
				if (c == '{') {
					...
					// If we didn't find the '{' character, find the end of
					// the
					// variable...
				while (i < end) {
					c = array[i]; // Not needed the first iteration, but
			...
			case Token.LITERAL_CHAR:
				if (c == '\\') {
					backslash = !backslash; // Okay because if we got in
											// here, backslash was initially
											// false.
				} else {
					if (c == '\'' && !backslash) {
					...
					backslash = false; // Need to set backslash to false
										// here as a character was typed.
				}
			...
			case Token.LITERAL_BACKQUOTE:
				...
				case '`':
					if (!backslash) {
						addToken(text, currentTokenStart, i, Token.LITERAL_BACKQUOTE,
								newStartOffset + currentTokenStart);
						currentTokenType = Token.NULL;
						// backslash is definitely false when we leave.
						break;
				...
				case '$':
					...
					addToken(text, currentTokenStart, i - 1, Token.LITERAL_BACKQUOTE,
							newStartOffset + currentTokenStart);
					currentTokenType = Token.VARIABLE;
					currentTokenStart = i;
					// First check if the variable name is enclosed in
					// '{' and '}' characters.
					if (i < end - 1 && array[i + 1] == '{') {
						...
						while (++i < end) {
							if (array[i] == '}') {
								...
								} else { // i==end = "trick" this method
											// so that the string is
											// continued to the next
											// line.
									currentTokenStart = i;
									currentTokenType = Token.LITERAL_BACKQUOTE;
									break; // So we don't hit the
											// condition below.
								}
							} // End of if (array[i]=='}').
						} // End of while (++i<end).
						if (i == end) { // Happens when '}' wasn't
										// found...
							addToken(text, currentTokenStart, end - 1, Token.VARIABLE,
									newStartOffset + currentTokenStart);
						...
						// If we reached the end of the variable, get
						// out.
					if (currentTokenType == Token.NULL || currentTokenType == Token.LITERAL_BACKQUOTE)
						break;
					// If we didn't find the '{' character, find the end
					// of the variable...
					...
					// This only happens if we never found the end of
					// the variable in the loop above.
					// We "trick" this method so that the backquote
					// string token is at the end.
					if (i == end) {
						addToken(text, currentTokenStart, i - 1, Token.VARIABLE, newStartOffset + currentTokenStart);
						currentTokenStart = i;
						currentTokenType = Token.LITERAL_BACKQUOTE;
			...
			case Token.LITERAL_STRING_DOUBLE_QUOTE:
				...
				case '$':
					...
					// Add the string up-to the variable.
					addToken(text, currentTokenStart, i - 1, Token.LITERAL_STRING_DOUBLE_QUOTE,
							newStartOffset + currentTokenStart);
					currentTokenType = Token.VARIABLE;
					currentTokenStart = i;
					// First check if the variable name is enclosed in
					...
					// This only happens if we never found the end of
					// the variable in the loop above.
					// We "trick" this method so that the double-quote
					// string token is at the end.
					if (i == end) {
						addToken(text, currentTokenStart, i - 1, Token.VARIABLE, newStartOffset + currentTokenStart);
						currentTokenStart = i;
