...
public class RSyntaxDocument extends RDocument implements Iterable<Token>, SyntaxConstants {
	...
	protected void fireInsertUpdate(DocumentEvent e) {
		...
		if (added != null && added.length > 0) {
			...
			// System.err.println("... adding lines: " + line + " - " +
			// (endBefore-1));
			// System.err.println("... ... added: " + added.length + ",
			// removed:" +
			// numRemoved);
	...
	protected void fireRemoveUpdate(DocumentEvent chng) {
		...
		if (removed != null && removed.length > 0) {
			...
			// System.err.println("... removing lines: " + line + " - " +
			// (endBefore-1));
			// System.err.println("... added: " + numAdded + ", removed: " +
			// removed.length);
			lastTokensOnLines.removeRange(line, endBefore); // Removing values
															// for lines
															// [line-(endBefore-1)].
			// System.err.println("--------- lastTokensOnLines.size() == " +
			// lastTokensOnLines.getSize());
			// Update last tokens for lines below until they've stopped
			// changing.
			updateLastTokensBelow(line, numLines, previousTokenType);
			...
			int previousLine = line - 1;
			int previousTokenType = (previousLine > -1 ? lastTokensOnLines.get(previousLine) : Token.NULL);
			// System.err.println("previousTokenType for line : " + previousLine
			// + " is " +
			// previousTokenType);
			// Update last tokens for lines below until they've stopped
			// changing.
	...
	public final Token getTokenListForLine(int line) {
		...
		// int endOffset = (line==map.getElementCount()-1 ? elem.getEndOffset()
		// - 1:
		// elem.getEndOffset() - 1);
	...
	public void setSyntaxStyle(String styleKey) {
		if (styleKey.equals("text/javaassembly")) {
			tokenMaker = new JavaAssemblyTokenMaker();
		} else {
	...
	private int updateLastTokensBelow(int line, int numLines, int previousTokenType) {
		...
		// ")");
		while (line < end) {
			setSharedSegment(line); // Sets s's text to that of line 'line' in
									// the document.
			...
			if (oldTokenType == newTokenType) {
				// System.err.println("... ... ... repainting lines " +
				// firstLine + "-" + line);
			...
			// FIXME: We currently cheat and send the line range that needs to
			// be
			...
			// length.
		if (line > firstLine) {
			// System.err.println("... ... ... repainting lines " + firstLine +
			// "-" + line);
			fireChangedUpdate(new DefaultDocumentEvent(firstLine, line, DocumentEvent.EventType.CHANGE));
		}
		return line;
