 ...
 * whitespace, the fifth <code>=</code>, etc.
 * <p>
 *
...
public interface Token extends TokenTypes {
	...
	/**
	 * Appends HTML code for painting this token, using the given text area's color
	 * scheme.
	 *
	 ...
	 *                   token. You can pass <code>false</code> for this parameter
	 *                   if, for example, you are making all your HTML be
	 *                   monospaced, and don't want any crazy fonts being used in
	 *                   the editor to be reflected in your HTML.
	...
	/**
	 * Returns the position in the token's internal char array corresponding to the
	 * specified document position.
	 * <p>
	 * Note that this method does NOT do any bounds checking; you can pass in a
	 * document position that does not correspond to a position in this token, and
	 * you will not receive an Exception or any other notification; it is up to the
	 * caller to ensure valid input.
	 *
	...
	/**
	 * Returns the end offset of this token in the document (exclusive). In other
	 * words, the token ranges from <code>[getOffset(), getEndOffset())</code>.
	 *
	...
	/**
	 * Returns a <code>String</code> containing HTML code for painting this token,
	 * using the given text area's color scheme.
	 *
	 * @param textArea The text area whose color scheme to use.
	 ...
	 *         language, any positive value denotes a specific secondary language.
	 ...
	 *
	 * @return The last non-comment, non-whitespace token, or <code>null</code> if
	 *         there isn't one.
	 */
	Token getLastNonCommentNonWhitespaceToken();

	/**
	 * Returns the last paintable token in this token list, or <code>null</code> if
	 * there is no paintable token.
	 *
	 ...
	 * <p>
	 *
	 * Note that this method isn't used much by the <code>ryntaxtextarea</code>
	 ...
	 * location <code>x0</code>
	 * <p>
	 * . This method will return the document position "closest" to the x-coordinate
	 * (i.e., if they click on the "right-half" of the <code>w</code> in
	 * <code>awe</code>, the caret will be placed in between the <code>w</code> and
	 * <code>e</code>; similarly, clicking on the left-half places the caret between
	 * the <code>a</code> and <code>w</code>). This makes it useful for methods such
	 * as <code>viewToModel</code> found in <code>javax.swing.text.View</code>
	 * subclasses.
	 *
	 ...
	 *                 <code>tokenList</code>.
	 * @param x        The pixel-position for which you want to get the
	 *                 corresponding offset.
	 * @return The position (in the document, NOT into the token list!) that covers
	 *         the pixel location. If <code>tokenList</code> is <code>null</code> or
	 *         has type <code>Token.NULL</code>, then <code>-1</code> is returned;
	 *         the caller should recognize this and return the actual end position
	 *         of the (empty) line.
	 */
	...
	/**
	 * Returns the position in the document that represents the last character in
	 * the token that will fit into <code>endBeforeX-startX</code> pixels. For
	 * example, if you're using a monospaced 8-pixel-per-character font, have the
	 * token "while" and <code>startX</code> is <code>0</code> and
	 ...
	 * <code>24</code>, while the "l" ends at <code>32</code>. If not even the first
	 * character fits in <code>endBeforeX-startX</code>, the first character's
	 * position is still returned so calling methods don't go into infinite loops.
	 *
	 ...
	 * @return The last document position that will fit in the specified amount of
	 *         pixels.
	 */
	/*
	 * @see #getTokenListOffsetBeforeX FIXME: This method does not compute
	 * correctly! It needs to be abstract and implemented by subclasses.
	 */
	int getOffsetBeforeX(RSyntaxTextArea textArea, TabExpander e, float startX, float endBeforeX);

	/**
	 * Returns the character array backing the lexeme of this token. This value
	 ...
	 * lexeme.
	 * <p>
	 * This method is preferred over the other overload in performance-critical code
	 * where this operation may be called frequently, since it does not involve any
	 * String allocations.
	 *
	 ...
	 * performance-critical code, as this one involves a String allocation while the
	 * other does not.
	 *
	 * @param type   The type to check for.
	 * @param lexeme The lexeme to check for.
	...
	/**
	 * Returns whether this token is a {@link #SEPARATOR} representing a single left
	 * curly brace.
	 *
	...
	/**
	 * Returns whether or not this token is "paintable;" i.e., whether or not the
	 * type of this token is one such that it has an associated syntax style. What
	 * this boils down to is whether the token type is greater than
	...
	/**
	 * Returns the bounding box for the specified document location. The location
	 * must be in the specified token list; if it isn't, <code>null</code> is
	 * returned.
	 *
	 * @param textArea The text area from which the token list was derived.
	 * @param e        How to expand tabs.
	 ...
	 *                      denotes the "main" language, any positive value denotes
	 *                      a specific secondary language. Negative values will be
	 *                      treated as <code>0</code>.
	...
	/**
	 * Returns the position in the document corresponding to the specified position
	 * in this token's internal char array (<code>textOffset</code> -
	 * <code>textOffset+textCount-1</code>).
	 * <p>
	 * Note that this method does NOT do any bounds checking; you can pass in an
	 * invalid token position, and you will not receive an Exception or any other
	 * indication that the returned document position is invalid. It is up to the
	 * user to ensure valid input.
	 *
