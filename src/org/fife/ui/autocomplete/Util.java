...
public class Util {
	 ...
	 * Otherwise, if a Substance Look and Feel is installed, we will attempt to use
	 * Substance cell renderers in all of our dropdowns.
	 * <p>
	 *
	 * Note that we do not have a build dependency on Substance, so all access to
	 * Substance stuff is done via reflection. We will fall back onto default
	 ...
	 *
	 * @param uri The URI to open. If this is <code>null</code>, nothing happens and
	 ...
	 * <code>null</code> if it is unsupported on this platform (or the JRE is older
	 * than 1.6).
	 *
	...
	/**
	 * Give apps a chance to decorate us with drop shadows, etc. Since very scrolly
	 * things such as lists (of e.g. completions) are *very* slow when in per-pixel
	 * translucent windows, even as of Java 7u2, we force the user to specify an
	 * extra option for the two "main" auto-complete windows.
	 *
	...
	/**
	 * Returns whether we should attempt to use Substance cell renderers and styles
	 * for things such as completion choices, if a Substance Look and Feel is
	 * installed. If this is <code>false</code>, we'll use our standard rendering
	 * for completions, even when Substance is being used.
	 *
	...
	/**
	 * Returns whether a string starts with a specified prefix, ignoring case. This
	 * method does not support characters outside of the BMP.
	 *
