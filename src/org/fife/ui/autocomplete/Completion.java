 ...
 * choice.
 * <li>The text that will be filled in if the user chooses this completion. Note
 * that often, this is the same as the text the user must (begin to) enter, but
 * this doesn't have to be the case.
 * <li>Summary HTML that describes this completion. This is information that can
 * be displayed in a helper "tooltip"-style window beside the completion list.
 * This may be <code>null</code>. It may also be lazily generated to cut down on
 * memory usage.
...
public interface Completion extends Comparable<Completion> {
	...
	/**
	 * Returns the portion of this completion that has already been entered into the
	 * text component. The match is case-insensitive.
	 * <p>
	 *
	...
	/**
	 * Returns the text that the user has to (start) typing for this completion to
	 * be offered. Note that this will usually be the same value as
	 * {@link #getReplacementText()}, but not always (a completion could be a way to
	 ...
	 * different things to different languages, and may depend on the context of the
	 * completion.
	 * <p>
	 *
	 * By default, all completions have a relevance of <code>0</code>. The higher
	 * the value returned by this method, the higher up in the list this completion
	 * will be; the lower the value returned, the lower it will be.
	 * <code>Completion</code>s with equal relevance values will be sorted
	 * alphabetically.
	 *
	...
	/**
	 * Returns the text to insert as the result of this auto-completion. This is the
	 * "complete" text, including any text that replaces what the user has already
	 * typed.
	 *
	...
	/**
	 * Returns the description of this auto-complete choice. This can be used in a
