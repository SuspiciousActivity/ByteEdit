...
public class LineNumberList extends AbstractGutterComponent implements MouseInputListener {
	...
	private int lastY = -1; // Used to check if caret changes lines when line
							// wrap is enabled.
	private int lastVisibleLine;// Last line index painted.
	private int cellHeight; // Height of a line number "cell" when word wrap is
							// off.
				...
				if (fm != null) {
					...
					while (fold != null && fold.isCollapsed()) {
						...
						if (hiddenLineCount == 0) {
							// Fold parser identified a 0-line fold region...
							// This
		...
		public void caretUpdate(CaretEvent e) {
			...
			if (!textArea.getLineWrap()) {
				...
				try {
					...
					if (y != lastY) {
						lastY = y;
						currentLine = textArea.getDocument().getDefaultRootElement().getElementIndex(dot);
