...
public abstract class RPrintUtilities {
	...
	private static int numDocLines; // The number of lines in the current
									// document.
	private static Element rootElement; // The first Element (line) in the
										// current document.
	...
	private static int getLineBreakPoint(String line, final int maxCharsPerLine) {
		...
		for (int i = 0; i < BREAK_CHARS.length; i++) {
			int breakCharPos = line.lastIndexOf(BREAK_CHARS[i], maxCharsPerLine - 1);
	...
	public static int printDocumentMonospaced(Graphics g, Document doc, int fontSize, int pageIndex,
		...
		int fontWidth = fm.charWidth('w'); // Any character will do as font is
											// monospaced.
		...
		// Since y is the baseline of where we'll start printing (not the
		// top-left
		...
		currentDocLineNumber = 0; // The line number of the document we're
									// currently on.
		rootElement = doc.getDefaultRootElement(); // To shorten accesses in our
													// loop.
		numDocLines = rootElement.getElementCount(); // The number of lines in
														// our document.
		while (currentDocLineNumber < numDocLines) {
			...
			currentDocLineNumber += 1; // We have printed one more line from the
										// document.
			numPrintedLines++;
			if (numPrintedLines > startingLineNumber) {
	...
	public static int printDocumentMonospacedWordWrap(Graphics g, Document doc, int fontSize, int pageIndex,
		...
		// RPrintTabExpander tabExpander = new RPrintTabExpander();
		// Get width and height of characters in this monospaced font.
		int fontWidth = fm.charWidth('w'); // Any character will do here, since
											// font is monospaced.
		int fontHeight = fm.getHeight();
		int maxCharsPerLine = (int) pageFormat.getImageableWidth() / fontWidth;
		int maxLinesPerPage = (int) pageFormat.getImageableHeight() / fontHeight;
	...
	public static int printDocumentWordWrap(Graphics g, JTextComponent textComponent, Font font, int pageIndex,
		...
		currentDocLineNumber = 0; // The line number of the document we're
									// currently on.
		int startingOffset = 0; // Used when a line is so long it has to be
								// wrapped.
		while (currentDocLineNumber < numDocLines) {
			...
			// at our desired offset
			// (because this line may be the second part of a wrapped line, so
			...
			// System.err.println("'" + currentLineSeg + "' - " +
			// currentLineLengthInPixels + "/" + LINE_LENGTH_IN_PIXELS);
			// If it'll fit by itself on a printed line, great.
			if (currentLineLengthInPixels <= lineLengthInPixels) {
				currentDocLineNumber += 1; // We (will) have printed one more
											// line from the document.
				startingOffset = 0; // Start at the first character in the new
									// document line.
			}
			...
			// wrapped)...
				...
				while (currentLineLengthInPixels > lineLengthInPixels) {
					...
					for (int i = 0; i < BREAK_CHARS.length; i++) {
						...
						int pos = currentLineString.lastIndexOf(BREAK_CHARS[i]) + 1;
					...
					// So here, we set currentPos to be the position of the last
					// character that fits
				...
				} // End of while (currentLineLengthInPixels >
					// LINE_LENGTH_IN_PIXELS).
				startingOffset += currentPos; // Where to start (offset from
												// line's start), since this
												// line wraps.
			} // End of else.
			numPrintedLines++;
			if (numPrintedLines > startingLineNumber) {
