...
public class UnicodeReader extends Reader {
	...
	/**
	 * The encoding being used. We keep our own instead of using the string returned
	 * by <code>java.io.InputStreamReader</code> since that class does not return
	 * user-friendly names.
	 */
	 ...
	 *
	 * @param in              The input stream from which to read.
	 * @param defaultEncoding The encoding to use if no recognized BOM is found. If
	...
	/**
	 * Returns the encoding being used to read this input stream (i.e., the encoding
	 * of the file). If a BOM was recognized, then the specific Unicode type is
	 * returned; otherwise, either the default encoding passed into the constructor
	 * or the system default is returned.
	 *
	 ...
	 *
	 * @param defaultEncoding The encoding to use if no BOM was recognized. If this
	 *                        value is <code>null</code>, then a system default is
	 *                        used.
	...
	protected void init(InputStream in, String defaultEncoding) throws IOException {
		...
		if ((bom[0] == (byte) 0x00) && (bom[1] == (byte) 0x00) && (bom[2] == (byte) 0xFE) && (bom[3] == (byte) 0xFF)) {
			encoding = "UTF-32BE";
			unread = n - 4;
		} else if (n == BOM_SIZE && // Last 2 bytes are 0; could be an empty
									// UTF-16
				(bom[0] == (byte) 0xFF) && (bom[1] == (byte) 0xFE) && (bom[2] == (byte) 0x00)
				&& (bom[3] == (byte) 0x00)) {
			encoding = "UTF-32LE";
			unread = n - 4;
	...
	/**
	 * Read characters into a portion of an array. This method will block until some
